...
Clear-Host
$looptoggle = $true
Write-Host "================[Providing Service]================`r`n================[Q: Quit][P: Pause]================"
...
while ($looptoggle) {
    ...
    if ($oldItems -lt $newItems) { # Test if the number of previously counted lines differs from the current line count.
        ...
        $selectSheet = $openExcel.Sheets.Item("$((Get-Date -UFormat %b).ToUpper())") # This needs to check date and return abbreviation of the month
        ...
        Foreach ($line in $oldItems..$newItems) { 
            ...
            if ($entry[$line].Contains($bravo[1]) -or $entry[$line].Contains($bravo[2]) -and (-not $entry[$line].Split(" ")[0].Contains("$alpha"))) { 
                ...
                $selectSheet.Cells.Item($row,3).WrapText = "True"
                Write-Output "[$(Get-Date -Format "HH:mm")][FROM LINE: $($line + 1) TO ROW: $row][$newEntry]" >> "GGBOT_LOGS_$getdate.txt" # Add to the log
                Write-Host "[$(Get-Date -Format "HH:mm")][FROM LINE: $($line + 1) TO ROW: $row][$newEntry]" # Display on the console 

            }

            ### This part is freakin dangerous ###
            if ($entry[$line].Contains($bravo[3]) -and (-not $entry[$line].Split(" ")[0].Contains("$alpha"))) {
                
                while ( !($selectSheet.Cells.Item($row,2).Value2.Contains("$($entry[$line].Split(" ")[4])"))) { $row++ } # Make a default overwrite cell that positively doesn't have anything valuable in it...
                $selectSheet.Cells.Item($row,3).value2 += "`r`n$newEntry"
                Write-Output "[$(Get-Date -Format "HH:mm")][FROM LINE: $($line + 1) TO ROW: $row][$newEntry]" >> "GGBOT_LOGS_$getdate.txt" # Add to the log
                Write-Host "[$(Get-Date -Format "HH:mm")][FROM LINE: $($line + 1) TO ROW: $row][$newEntry]" # Display on the console     
            }
    ...
    if ([console]::KeyAvailable) {
        ...
        switch ($x.key) {
            "p" { Read-Host -Prompt "================[Press Enter to Resume]================" }
